...
class TexturalFilter():
    ...
    def __ngtdm_filter(
        ...
        if feature:
            if isinstance(feature, str):
                assert feature in self.ngtdm_features,\
                    "feature should be a string or an integer and should be one of the following: " + ", ".join(self.ngtdm_features) + "."
            elif isinstance(feature, int):
                assert feature in range(len(self.ngtdm_features)),\
                    "feature's index should be an integer between 0 and " + str(len(self.ngtdm_features) - 1) + "."
            else:
                raise TypeError("feature should be an integer or a string from the following list: " + ", ".join(self.ngtdm_features) + ".")
        ...
        if feature is not None:
            ...
            feature = self.ngtdm_features.index(feature) if isinstance(feature, str) else feature

            # Initialize the kernel
            kernel_ngtdm = single_ngtdm_kernel.substitute(
                max_vol=int(max_vol),
            ...
            # Initialize the kernel
            kernel_ngtdm = ngtdm_kernel.substitute(
                max_vol=int(max_vol),
                filter_size=self.size,
                shape_volume_0=int(volume_copy.shape[0]),
                shape_volume_1=int(volume_copy.shape[1]),
        ...
        mod = SourceModule(kernel_ngtdm, no_extern_c=True)
        if self.local:
            process_loop_kernel = mod.get_function("ngtdm_filter_local")
        else:
            process_loop_kernel = mod.get_function("ngtdm_filter_global")
    ...
    def __call__(
        ...
        # Filtering
        if self.family.lower() == "ngtdm":
            filtered_images = self.__ngtdm_filter(input_images, discretization, user_set_min_val, feature)
        else:
            raise NotImplementedError("Only ngtdm is implemented for now.")
